...
training_file_name = '/home/jliu/Desktop/files/research/reaction_detection/fictious_data/training_set/training_transition.txt'

#with open('timeseries_1.0.data', 'rb') as fil:
with open(training_file_name, 'rb') as fil:
    transition = pickle.load(fil)[:-1]
with open(args.file_name, 'rb') as fil:
    bo = pickle.load(fil)
...
#print bo[0][0][1:]
data = [ np.abs(bo[i] - bo[i-1]) for i, b in enumerate(bo[1:]) ]
#print data
...
#while total_loss > thresh:
    ...
    #print('db1', db1)
    dw1 = db1 * np.tile(avg, (3, 1))
    #print('dw1', dw1)
    #print np.sum(dw1, axis = 1).reshape(3, 1)
    w1 = w1 - alpha * np.sum(dw1, axis = 1).reshape(3, 1) / num_training
    b1 = b1 - alpha * np.sum(db1, axis = 1).reshape(3, 1) / num_training
    #print('w2', w2)
    #print np.sum(dw2, axis = 1).reshape(1, 3)
    w2 = w2 - alpha * np.sum(dw2, axis = 1).reshape(1, 3) / num_training
    #print('w2', w2)
...
plt.plot(transition, alpha = 0.8, label = 'transition')
plt.legend()
