...
void compute_plaquettes_through_lattice(vector<int> dimension_lengths, int lattice_size, site lattice[])
{
    for (int s = 1; s <= lattice_size; s++)
    {
        cout << compute_real_plaquettes_at_site(s, dimension_lengths, 0, lattice) << endl;
    }
}

void print_angles_through_lattice(vector<int> dimension_lengths, int lattice_size, site lattice[])
{
    for (int s = 0; s < lattice_size; s++)
    {
        for (int d = 0; d < dimension_lengths.size(); d++)
        {
            cout << arg(lattice[s].link[dimension_lengths[d]]) << endl;
        }
    }
}
...
//    compute_polyakov_pairs(dimension_lengths, polyakov_list);
    ...
    for (int traj = 1; traj <= traj_length; traj++)
    {
        ...
        {
            cout << "TRAJ " << traj << endl;
//            print_angles_through_lattice(dimension_lengths, lattice_size, lattice);
            compute_plaquettes_through_lattice(dimension_lengths, lattice_size, lattice);
//            compute_avg_plaquette(dimension_lengths, lattice_size, lattice);
