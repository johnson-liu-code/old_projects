...
def oneTimeStep(r, alive, N, psi, E_T, ESum, ESqdSum):
    for n in range(N):
        r, alive = oneMonteCarloStep(r, alive, n, N, E_T)
    #print(r, alive)
...
#        print('r: {}    r**dim: {}    psi: {}    psiNorm: {}'.format(r, r**dim, psi[i], psiNorm))
        psi_value = r**(float(dim-1.)) * psi[i] / psiNorm
        psiExact_value = r**(float(dim-1.)) * np.exp(-r**2.) / psiExactNorm
