...
//uniform_real_distribution<double> uniform_distr(0.0, 1.0);
...
void initialize() {
    ...
    for (int n = 0; n < N; n++) {
        ...
        for (int d = 0; d < DIM; d++)
            {
                r[n][d] = uniform_dist() - .5;
                //double number = uniform_distr(generator1);
                //r[n][d] = number - 0.5;
            }
        alive[n] = true;
    }
...
void oneMonteCarloStep(int n) {
    ...
    for (int d = 0; d < DIM; d++)
    {
        r[n][d] += normal_dist() * sqrt(dt);
        //double number = normal_distr(generator2);
        //r[n][d] += number * sqrt(dt);
    }
    ...
    int survivors = int(q);
    //cout << survivors << endl;
    //double number = uniform_distr(generator3);
    //if (q - survivors > number)
    if (q - survivors > uniform_dist())
        ++survivors;
...
void oneTimeStep() {
    ...
    //cout << sizeof(r) << '\t' << sizeof(alive) << endl;
