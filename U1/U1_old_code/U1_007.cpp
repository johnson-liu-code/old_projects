...
void initialize_lattice(site lattice[], int lattice_size, int number_of_dimensions, double init_angle)
{
    ...
    for (int s = 0; s < lattice_size; s++)
    {
        ...
        {
            lattice[s].link.push_back(i);
            i++;
//            lattice[s].link.push_back(init_angle);
//            lattice[s].link.push_back( (s+1) * l * M_PI);
//            cout << "site: " << s + 1 << ", dimension: " << l << ", angle: " << lattice[s].link[l] << endl;
        }
    }
}
...
//            cout << "d2: " << d2 << endl;
    ...
    if (index == 8)
    {
        cout << "site: " << index << ", dimension: " << direction << ", staples: " << staples << endl;
    }
    return staples;
}
